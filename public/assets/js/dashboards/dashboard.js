...
//import './bootstrap';
//https://www.dlitz.net/software/addrow/
function addRow() {
  /* Declare variables */
  ...
  elements = document.getElementsByTagName("addIngredients");
  templateRow = null;
  rowCount = 0;
  ...
  for (i = 0; i < elements.length; i++) {
    ...
    if (className == null && row.attributes) {
      ...
      if (className && _typeof(className) == 'object' && className.value) {
        // MSIE 6
  ...
  if (templateRow == null) return false;
  ...
  for (i = 0; i < elements.length; i++) {
    ...
    s = null;
    s = element.getAttribute("Ingredients_Name");
    if (s == null) continue;
    t = s.split("[");
    if (t.length < 2) continue;
    s = t[0] + "[" + rowCount.toString() + "]";
    element.setAttribute("Ingredients_Name", s);
    element.value = "";
  }
